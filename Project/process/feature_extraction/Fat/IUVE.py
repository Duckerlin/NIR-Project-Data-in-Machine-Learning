...
# IUVE 波長選擇函數
def iuve_wavelength_selection(X, y, n_components, n_noise_vars=PARAMS['UVE_NOISE_VARIABLES'], cv_folds=PARAMS['UVE_CV_FOLDS'], alpha=1.0):
    """
    改進的無信息變量消除 (IUVE) 用於選擇重要波長，基於嶺回歸解決多重共線性問題
    參數：
        X: DataFrame or ndarray, samples x wavelengths
        y: Target variable
        n_components: Assumed PLS components (for compatibility, uses Ridge regression)
        n_noise_vars: Number of random noise variables
    ...
    if n_features < 1:
        print("  No features available for IUVE, returning empty list")
        return []
    
    # 生成隨機噪聲變量，範圍與標準化後的光譜數據一致
    noise = np.random.normal(0, 1e-6, size=(n_samples, n_noise_vars))
    X_augmented = np.hstack((X_array, noise))
    ...
    for fold, (train_idx, _) in enumerate(kf.split(X_augmented)):
        ...
        try:
            ...
            print(f"  Ridge regression failed in fold {fold}: {e}")
            coefs[fold] = np.zeros(X_augmented.shape[1])
    ...
    # 使用噪聲變量的穩定性閾值選擇重要波長（0.99倍最大絕對值）
    if len(stability_noise) == 0:
        print("  No noise variables generated, using default threshold")
        threshold = np.max(np.abs(stability_real)) * 0.99
    else:
...
# === 步驟 6: 無信息變量消除（IUVE）波長選擇 ===
# 使用 IUVE 選擇重要波長
selected_wavelength_indices = iuve_wavelength_selection(X_train_preprocessed, Y_train, n_components=PARAMS['MCS_N_COMPONENTS'])
...
print(f"  IUVE 選擇了 {len(selected_wavelength_indices)} 個波長: {selected_wavelengths}")
...
plt.scatter(selected_wavelengths, mean_spectrum[selected_wavelength_indices], 
           color='red', marker='o', label='IUVE Selected Wavelengths', zorder=5)
plt.xlabel('Wavelength (nm)', fontsize=14)
plt.ylabel('Absorbance (Log(1/R))', fontsize=14)
plt.title('IUVE Selected Wavelengths on Mean NIR Spectrum', fontsize=16, pad=10)
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
...
plt.title('PLS Regression: Actual vs Predicted Fat (IUVE)', fontsize=16, pad=10)
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
...
specific_params = {
    ...
    'Number of Selected Wavelengths (IUVE)': len(selected_wavelength_indices),
...
base_filename = 'PLS_Fat_MAS_Centering_IUVE'
counter = 1
