...
PARAMS = {
    ...
    'UVE_NOISE_VARIABLES': 50,  # UVE 隨機噪聲變量數量
...
def uve_wavelength_selection(X, y, n_components, n_noise_vars=PARAMS['UVE_NOISE_VARIABLES'], cv_folds=PARAMS['UVE_CV_FOLDS']):
    ...
    if n_features < 1:
        print("  No features available for UVE, returning empty list")
        return []
    ...
    # 初始化 PLS 模型
    pls = PLSRegression(n_components=min(n_components, X_augmented.shape[1], n_samples))
    ...
    for fold, (train_idx, _) in enumerate(kf.split(X_augmented)):
        ...
        try:
            ...
            print(f"  PLS regression failed in fold {fold}: {e}")
    ...
    if len(stability_noise) == 0:
        ...
        threshold = np.percentile(stability_real, 50)
    else:
        threshold = np.percentile(stability_noise, 50)
...
# 使用 UVE 選擇重要波長
selected_wavelength_indices = uve_wavelength_selection(X_train_preprocessed, Y_train, n_components=PARAMS['MCS_N_COMPONENTS'])
...
print(f"  UVE 選擇了 {len(selected_wavelength_indices)} 個波長: {selected_wavelengths}")
...
plt.scatter(selected_wavelengths, mean_spectrum[selected_wavelength_indices], 
           color='red', marker='o', label='UVE Selected Wavelengths', zorder=5)
plt.xlabel('Wavelength (nm)', fontsize=14)
plt.ylabel('Absorbance (Log(1/R))', fontsize=14)
plt.title('UVE Selected Wavelengths on Mean NIR Spectrum', fontsize=16, pad=10)
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
...
plt.title('PLS Regression: Actual vs Predicted Fat (UVE)', fontsize=16, pad=10)
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
...
base_filename = 'PLS_Fat_MAS_Centering_UVE'
counter = 1
