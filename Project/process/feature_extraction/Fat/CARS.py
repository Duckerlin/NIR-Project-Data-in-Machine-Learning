...
PARAMS = {
    ...
    'CARS_N_FEATURES': 16,  # CARS 特徵波長數量
    'CARS_N_ITERATIONS': 50,  # CARS 抽樣次數
...
# === 步驟 3: NIR 光譜預處理（移除標準化，移到步驟 5 結合 MAS 和中心化） ===
print("  跳過獨立標準化，將在步驟 5 結合移動平均平滑與中心化")
...
# === 步驟 6: 特徵波長提取（CARS） ===
def cars(X, y, n_features=PARAMS['CARS_N_FEATURES'], n_iterations=PARAMS['CARS_N_ITERATIONS']):
    """
    CARS (Competitive Adaptive Reweighted Sampling) for wavelength selection.
    參數：
        X: DataFrame or ndarray, samples x wavelengths
        y: Target variable
        n_features: Number of wavelengths to select
        n_iterations: Number of CARS iterations
    返回：
        final_indices: Selected wavelength indices
    """
    if isinstance(X, pd.DataFrame):
        X_array = np.array(X)
        columns = X.columns
    ...
    if n_features >= n_wavelengths:
        print(f"  n_features ({n_features}) >= n_wavelengths ({n_wavelengths}), returning all indices")
        return np.arange(n_wavelengths)
    ...
    for _ in range(n_iterations):
        ...
        pls.fit(X_array[:, selected_indices], y)
        coef = np.abs(pls.coef_.ravel())
        if np.sum(coef) == 0:
            print("  PLS coefficients are all zero, stopping CARS iteration")
            break
    ...
    if not selected_history:
        print("  No valid iterations in CARS, returning initial indices")
        return np.arange(n_wavelengths)[:n_features]
...
# Apply CARS to preprocessed data
selected_wavelength_indices = cars(X_train_preprocessed, Y_train)
...
print(f"  CARS 選擇 {len(selected_wavelength_indices)} 個特徵波長")
print(f"  CARS 選擇的波長: {nir_final_df['Wavelength'].values[selected_wavelength_indices]}")

# 繪製特徵波長選擇圖
plt.figure(figsize=(10, 6))
mean_spectrum = X_train.mean().values  # Use raw spectrum mean for visualization
plt.plot(nir_final_df['Wavelength'], mean_spectrum, label='Mean Spectrum')
plt.scatter(nir_final_df['Wavelength'].values[selected_wavelength_indices], 
           mean_spectrum[selected_wavelength_indices], 
           color='red', label='Selected Wavelengths', zorder=5)
plt.xlabel('Wavelength (nm)', fontsize=14)
plt.ylabel('Absorbance (Log(1/R))', fontsize=14)
plt.title('CARS Selected Wavelengths for Fat', fontsize=16, pad=10)
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
...
plt.title('PLS Regression: Actual vs Predicted Fat', fontsize=16, pad=10)
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
...
common_params = {
    ...
    'CARS Feature Wavelengths': PARAMS['CARS_N_FEATURES'],
    'CARS Sampling Iterations': PARAMS['CARS_N_ITERATIONS'],
    'CARS Selected Wavelengths (nm)': ', '.join([f'{w:.1f}' for w in nir_final_df['Wavelength'].values[selected_wavelength_indices]]),
...
base_filename = 'PLS_Fat_MAS_Centering_CARS'
counter = 1
