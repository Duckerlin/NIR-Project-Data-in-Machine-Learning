...
PARAMS = {
    ...
    'SPA_MAX_WAVELENGTHS': 22,  # SPA 最大選擇波長數
    'MAS_WINDOW_SIZE': 5,  # MAS 窗口大小（奇數）
}
...
# SPA 演算法實現
def spa(X, y, max_wavelengths=PARAMS['SPA_MAX_WAVELENGTHS']):
    """
    Successive Projections Algorithm (SPA) for wavelength selection.
    參數：
        X: DataFrame or ndarray, samples x wavelengths
        y: Target variable
        max_wavelengths: Maximum number of wavelengths to select
    返回：
        selected_indices: Indices of selected wavelengths
    """
    ...
    max_wavelengths = min(max_wavelengths, n_wavelengths)
    
    if max_wavelengths < 1:
        print("  max_wavelengths must be at least 1, returning empty list")
        return []
    ...
    # 計算初始波長（選擇與 y 相關性最高的波長）
    correlations = np.abs(np.corrcoef(X_array.T, y)[:-1, -1])
    if np.all(np.isnan(correlations)):
        print("  All correlations are NaN, selecting first wavelength")
        first_idx = 0
    else:
        first_idx = np.argmax(correlations)
    selected_indices.append(first_idx)
    remaining_indices.remove(first_idx)
    
    # 正交投影
    X_selected = X_array[:, [first_idx]]
    
    for _ in range(max_wavelengths - 1):
        if not remaining_indices:
            break
        projections = np.zeros(len(remaining_indices))
        for i, idx in enumerate(remaining_indices):
            # 計算當前波長對已選波長的投影殘差
            x = X_array[:, idx].reshape(-1, 1)
            try:
                proj = x - X_selected @ np.linalg.pinv(X_selected) @ x
                projections[i] = np.linalg.norm(proj)
            except np.linalg.LinAlgError:
                projections[i] = 0
                print(f"  Singular matrix encountered for wavelength index {idx}")
        
        if np.all(projections == 0):
            print("  All projections are zero, stopping SPA")
            break
        next_idx = remaining_indices[np.argmax(projections)]
...
if 'Fat' in trad_df.columns:
    trad_df = trad_df.dropna(subset=['Fat'])
    Y = trad_df['Fat'].values
    print("\n  目標值 Y (Fat) 前五筆:", Y[:5])
    print("\n  目標值 Y (Fat) 描述:", pd.Series(Y).describe())
else:
    print("  缺少 'Fat' 欄位，無法建立模型")
    exit()
...
print(f"  剔除後目標值 Y (Fat) 描述:", pd.Series(Y).describe())

# 脂肪含量範圍檢查（0-50%）
fat_mask = (Y >= 0) & (Y <= 50)
if not np.all(fat_mask):
    print(f"  檢測到脂肪含量異常值（應在 0-50%）：{Y[~fat_mask]}")
    X = X[fat_mask]
    Y = Y[fat_mask]
    print(f"  移除脂肪異常值後，剩餘樣本數：{X.shape[0]}")
    print(f"  移除後目標值 Y (Fat) 描述:", pd.Series(Y).describe())
...
plt.title('Distribution of Fat Content', fontsize=16, pad=10)
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()

# === 步驟 3: NIR 光譜預處理（移除 Autoscaling，移到步驟 5 結合 MAS 和中心化） ===
print("  跳過獨立 Autoscaling，將在步驟 5 結合移動平均平滑與中心化")
...
def spxy(X, Y, test_size=PARAMS['TRAIN_TEST_RATIO']):
    ...
    dist_X = pairwise_distances(X_array)
    ...
    X_train = X_array[selected]
    Y_train = Y[selected].ravel()
    X_test = X_array[remaining]
...
print(f"  訓練集目標值 Y (Fat) 描述:", pd.Series(scaler_y.inverse_transform(Y_train.reshape(-1, 1)).ravel()).describe())
print(f"  測試集目標值 Y (Fat) 描述:", pd.Series(scaler_y.inverse_transform(Y_test.reshape(-1, 1)).ravel()).describe())

# === 步驟 5: 光譜預處理（結合移動平均平滑與中心化） ===
# 首先應用移動平均平滑（MAS）
X_train_mas = mas(X_train, window_size=PARAMS['MAS_WINDOW_SIZE'])
X_train_mas = pd.DataFrame(X_train_mas, index=X_train.index, columns=X_train.columns)
X_test_mas = mas(X_test, window_size=PARAMS['MAS_WINDOW_SIZE'])
X_test_mas = pd.DataFrame(X_test_mas, index=X_test.index, columns=X_test.columns)

# 隨後應用中心化（僅移除均值，不進行標準差縮放）
scaler_x = StandardScaler(with_std=False)  # 僅中心化，不縮放
X_train_preprocessed = scaler_x.fit_transform(X_train_mas)
X_test_preprocessed = scaler_x.transform(X_test_mas)

# 將預處理後的數據轉為 DataFrame，保留索引和欄名
X_train_preprocessed = pd.DataFrame(X_train_preprocessed, index=X_train.index, columns=X_train.columns)
X_test_preprocessed = pd.DataFrame(X_test_preprocessed, index=X_test.index, columns=X_test.columns)

print("  光譜預處理完成：移動平均平滑與中心化結合")

# === 步驟 6: SPA 特徵波長選擇 ===
selected_wavelength_indices = spa(X_train_preprocessed, Y_train)
wavelengths = nir_final_df['Wavelength'].values
selected_wavelengths = wavelengths[selected_wavelength_indices]
X_train_selected = X_train_preprocessed.iloc[:, selected_wavelength_indices]
X_test_selected = X_test_preprocessed.iloc[:, selected_wavelength_indices]
print(f"  SPA 選擇了 {len(selected_wavelength_indices)} 個波長: {selected_wavelengths}")
print(f"  訓練集形狀: {X_train_selected.shape}, 測試集形狀: {X_test_selected.shape}")

# 繪製選擇的波長
plt.figure(figsize=(10, 6))
mean_spectrum = X_train.mean().values  # 使用原始數據均值光譜進行可視化
plt.plot(wavelengths, mean_spectrum, label='Mean Spectrum')
plt.scatter(selected_wavelengths, mean_spectrum[selected_wavelength_indices], 
           color='red', marker='o', label='SPA Selected Wavelengths', zorder=5)
plt.xlabel('Wavelength (nm)', fontsize=14)
plt.ylabel('Absorbance (Log(1/R))', fontsize=14)
plt.title('SPA Selected Wavelengths on Mean NIR Spectrum', fontsize=16, pad=10)
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
...
table.scale(1.2, 1.2)
plt.title('Evaluation Metrics for Training and Test Sets (Fat)', fontsize=16, pad=10)
plt.tight_layout()
plt.show()
...
plt.xlabel('Actual Fat Content (%)', fontsize=14)
plt.ylabel('Predicted Fat Content (%)', fontsize=14)
plt.title('PLS Regression: Actual vs Predicted Fat (SPA)', fontsize=16, pad=10)
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
...
common_params = {
    ...
    'SPA Max Wavelengths': PARAMS['SPA_MAX_WAVELENGTHS'],
    'Preprocessing Method': 'MAS + Centering',
...
specific_params = {
    ...
    'Selected Wavelengths (nm)': ', '.join([f'{w:.1f}' for w in selected_wavelengths])
}
...
base_filename = 'PLS_Fat_MAS_Centering_SPA'
counter = 1
