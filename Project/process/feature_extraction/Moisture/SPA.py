...
PARAMS = {
    ...
    'SPA_MAX_WAVELENGTHS': 27,  # SPA 最大選擇波長數
}
...
def spa(X, y, max_wavelengths=PARAMS['SPA_MAX_WAVELENGTHS']):
    ...
    Parameters:
    X : numpy array, shape (n_samples, n_wavelengths)
        Spectral data.
    y : numpy array, shape (n_samples,)
        Target values.
    max_wavelengths : int
        Maximum number of wavelengths to select.
...
print("  光譜預處理完成：Savitzky-Golay 濾波與 Autoscaling 結合")

# === 步驟 6: SPA 特徵波長選擇 ===
# 應用 SPA 選擇特徵波長
selected_wavelength_indices = spa(X_train_preprocessed, Y_train, max_wavelengths=PARAMS['SPA_MAX_WAVELENGTHS'])
wavelengths = nir_final_df['Wavelength'].values
selected_wavelengths = wavelengths[selected_wavelength_indices]
X_train_selected = X_train_preprocessed[:, selected_wavelength_indices]
X_test_selected = X_test_preprocessed[:, selected_wavelength_indices]
...
mean_spectrum = scaler_x.inverse_transform(np.mean(X_train_sg, axis=0).reshape(1, -1))[0]
...
plt.title('SPA Selected Wavelengths on Mean NIR Spectrum (Moisture)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
table.scale(1.2, 1.2)
plt.title('Evaluation Metrics for Training and Test Sets (Moisture)')
plt.show()
...
plt.title('PLS Regression: Actual vs Predicted Moisture (SPA)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
common_params = {
    ...
    'Preprocessing Method': 'SGS + Autoscaling'
}
...
base_filename = 'PLS_Moisture_SGS_Autoscaling_SPA'
counter = 1
