...
PARAMS = {
    ...
    'UVE_NOISE_VARIABLES': 50,  # IUVE 隨機噪聲變量數量
    'UVE_CV_FOLDS': 5,  # IUVE 交叉驗證折數
}
...
plt.scatter(nir_final_df['Wavelength'].values[selected_wavelength_indices], mean_spectrum[selected_wavelength_indices], 
            color='red', label='IUVE Selected Wavelengths', zorder=5)
plt.xlabel('Wavelength (nm)')
plt.ylabel('Absorbance (Log(1/R))')
plt.title('IUVE Selected Wavelengths for Moisture')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
common_params = {
    ...
    'IUVE Selected Wavelengths (nm)': ', '.join([f'{w:.1f}' for w in nir_final_df['Wavelength'].values[selected_wavelength_indices]])
}
...
base_filename = 'PLS_Moisture_SGS_Autoscaling_IUVE'
counter = 1
