...
def uve_wavelength_selection(X, y, n_components, n_noise_vars=PARAMS['UVE_NOISE_VARIABLES'], cv_folds=PARAMS['UVE_CV_FOLDS']):
    ...
    # 使用噪聲變量的穩定性閾值選擇重要波長
    threshold = np.percentile(stability_noise, 90)  # 使用 90% 分位數作為閾值
...
plt.scatter(nir_final_df['Wavelength'].values[selected_wavelength_indices], mean_spectrum[selected_wavelength_indices], 
            color='red', label='UVE Selected Wavelengths', zorder=5)
plt.xlabel('Wavelength (nm)')
plt.ylabel('Absorbance (Log(1/R))')
plt.title('UVE Selected Wavelengths for Moisture')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
common_params = {
    ...
    'UVE Selected Wavelengths (nm)': ', '.join([f'{w:.1f}' for w in nir_final_df['Wavelength'].values[selected_wavelength_indices]])
}
...
base_filename = 'PLS_Moisture_SGS_Autoscaling_UVE'
counter = 1
