...
# === 步驟 6: SPA 特徵波長選擇 ===
selected_wavelength_indices = spa(X_train, Y_train, max_wavelengths=PARAMS['SPA_MAX_WAVELENGTHS'])
...
# 繪製選擇的波長（使用原始數據均值）
X_mean = nir_transposed_df.loc[common_ids].mean()  # 使用對齊後的原始數據均值
plt.figure(figsize=(10, 6))
plt.plot(wavelengths, X_mean.values, label='Mean Spectrum')
plt.scatter(selected_wavelengths, X_mean.values[selected_wavelength_indices], 
            color='red', marker='o', label='SPA Selected Wavelengths')
plt.xlabel('Wavelength (nm)')
plt.ylabel('Absorbance')
plt.title('SPA Selected Wavelengths on Mean NIR Spectrum')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
plt.title('PLS Regression: Actual vs Predicted Ash (SPA)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'PLS_Ash_SPA_Centering_{timestamp}.xlsx'
