...
# 定義統一參數
PARAMS = {
    'WAVELENGTH_RANGE': (650, 1099.5),  # 波長範圍 (min, max)
    'WAVELENGTH_RANGE_STR': '650-1099.5',  # 波長範圍字串表示
    ...
    'UVE_NOISE_VARIABLES': 600,  # UVE 隨機噪聲變量數量
...
def center(X):
    X = np.array(X)
    means = np.mean(X, axis=1, keepdims=True)  # 沿波長軸中心化
    X_centered = X - means
    return X_centered
...
def iuve_wavelength_selection(X, y, n_components, n_noise_vars=PARAMS['UVE_NOISE_VARIABLES'], cv_folds=PARAMS['UVE_CV_FOLDS'], alpha=1.0):
    ...
    X: 光譜數據 (樣本 x 波長)
    y: 目標值
    n_components: 假設的PLS主成分數（用於兼容性，實際上使用嶺回歸）
...
md_threshold = np.mean(md_thresholds) if md_thresholds else np.inf
...
def monte_carlo_outlier_detection(X, y, n_iterations=PARAMS['MCS_N_ITERATIONS'], 
    ...
    residuals = residuals / max(1, n_iterations)
...
if not np.all(ash_mask):
    print(f"  檢測到灰分含量異常值（應在 0-10%）：{Y[~ash_mask]}")
    X = X[ash_mask]
    Y = Y[ash_mask]
...
# 檢查樣本數是否足夠
if X.shape[0] < 5:
    print(f"  樣本數 {X.shape[0]} 過少，無法進行後續分析。請檢查數據。")
    exit()
...
# 檢查是否選擇到波長
if len(selected_wavelength_indices) == 0:
    print("  IUVE 未選擇到任何波長，使用所有波長繼續")
    selected_wavelength_indices = np.arange(X_train.shape[1])
...
max_lv = min(10, X_train_selected.shape[1], X_train_selected.shape[0] - 1)
if max_lv < 1:
    print(f"  可用的潛在變數數量為 {max_lv}，數據不足以進行PLS建模")
    exit()
...
for lv in range(1, max_lv + 1):
    ...
    try:
        scores = cross_val_score(
            ...
            X_train_selected,
            Y_train,
            cv=KFold(n_splits=min(5, X_train_selected.shape[0]), shuffle=True, random_state=42),
            scoring='neg_root_mean_squared_error'
        )
        rmse_cv.append(-scores.mean())
    except ValueError as e:
        print(f"  交叉驗證 (n_components={lv}) 失敗: {e}")
        continue

if not rmse_cv:
    print("  交叉驗證未能生成有效結果，設置默認 n_components=1")
    best_lv = 1
else:
...
# 5-fold 交叉驗證評估
cv_scores = cross_val_score(pls, X_train_selected, Y_train, cv=min(5, X_train_selected.shape[0]), scoring='r2')
print(f"\n  {min(5, X_train_selected.shape[0])}-fold Cross-Validation R² Scores: {cv_scores}")
...
plt.title('PLS Regression: Actual vs Predicted Ash (IUVE)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'PLS_Ash_IUVE_Centering_{timestamp}.xlsx'
