...
PARAMS = {
    ...
    'CARS_N_FEATURES': 14,  # CARS 特徵波長數量
...
if not np.all(ash_mask):
    print(f"  檢測到灰分含量異常值（應在 0-50%）：{Y[~ash_mask]}")
    X = X[ash_mask]
    Y = Y[ash_mask]
...
def cars(X, y, n_features=PARAMS['CARS_N_FEATURES'], n_iterations=PARAMS['CARS_N_ITERATIONS']):
    ...
    for _ in range(n_iterations):
        ...
        if n_components < 1 or len(selected_indices) < n_features:
            break
        X_array = np.array(X)  # 將 DataFrame 轉為 numpy 陣列
        ...
        selected_indices = np.random.choice(selected_indices, size=n_select, replace=False, p=weights[selected_indices] / np.sum(weights[selected_indices]))
...
selected_wavelengths = cars(X_train, Y_train)
X_train_selected = X_train.iloc[:, selected_wavelengths]
X_test_selected = X_test.iloc[:, selected_wavelengths]
...
plt.title('CARS Selected Wavelengths for Ash')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'PLS_Ash_Centering_{timestamp}.xlsx'
