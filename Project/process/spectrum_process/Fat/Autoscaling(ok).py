...
# === 可修改的目標欄位與範圍設定 ===
TARGET_COLUMN = 'Fat'           # 要預測的欄位名稱，改成 'Fat' 或 'Moisture' 等
VALUE_RANGE = (0, 100)         # 允許的目標值範圍（例如百分比 0-100）
...
PARAMS = {
    ...
    'TRAIN_TEST_RATIO': 0.2,  # 測試集比例 (0.25 表示 75:25)
    'TRAIN_TEST_RATIO_STR': '75:25',  # 訓練-測試比例字串表示
}
...
# 處理光譜資料（波長範圍）
# 假設 NIR 資料第一欄為波長，其餘欄為樣本或相反（依照原始腳本邏輯）
...
# 使用 TARGET_COLUMN 作為目標值
if TARGET_COLUMN in trad_df.columns:
    trad_df = trad_df.dropna(subset=[TARGET_COLUMN])
    Y = trad_df[TARGET_COLUMN].values
    print(f"\n 目標值 Y ({TARGET_COLUMN}) 前五筆:", Y[:5])
    print(f"\n 目標值 Y ({TARGET_COLUMN}) 描述:", pd.Series(Y).describe())
else:
    print(f" 缺少 '{TARGET_COLUMN}' 欄位，無法建立模型")
    exit()

# 轉置 NIR：假設原本為 Wavelength x Samples
...
if len(md_thresholds) == 0:
    print(" 無法計算 md_threshold，將使用預設值 3.0")
    md_threshold = 3.0
else:
...
# 目標值範圍檢查（使用通用 VALUE_RANGE）
value_mask = (Y >= VALUE_RANGE[0]) & (Y <= VALUE_RANGE[1])
if not np.all(value_mask):
    print(f" 檢測到 {TARGET_COLUMN} 含量異常值（應在 {VALUE_RANGE[0]}-{VALUE_RANGE[1]}）：{Y[~value_mask]}")
    X = X[value_mask]
    Y = Y[value_mask]
    print(f" 移除 {TARGET_COLUMN} 異常值後，剩餘樣本數：{X.shape[0]}")
    print(f" 移除後目標值 Y 描述:", pd.Series(Y).describe())

# 繪製目標值分佈直方圖
plt.figure(figsize=(8, 6))
plt.hist(Y, bins=20, edgecolor='black')
plt.xlabel(f'{TARGET_COLUMN} Content (%)')
plt.ylabel('Frequency')
plt.title(f'Distribution of {TARGET_COLUMN} Content')
plt.grid(True)
plt.tight_layout()
plt.show()

# === 步驟 3: 數據標準化（Autoscaling） ===
scaler_x = StandardScaler()
X_scaled = scaler_x.fit_transform(X)
X = pd.DataFrame(X_scaled, index=X.index, columns=X.columns)
print(" 數據標準化（Autoscaling）完成")
...
# === 步驟 5: 光譜預處理（使用現有 Autoscaling 數據） ===
# 直接使用步驟 3 的標準化數據，無需額外預處理
X_train_selected = X_train
X_test_selected = X_test
print(" 光譜預處理（使用現有 Autoscaling 數據）完成")
print(f" 使用 Autoscaling 數據，訓練集形狀: {X_train_selected.shape}, 測試集形狀: {X_test_selected.shape}")

# === 步驟 6: 交叉驗證找最佳 n_components ===
...
table.scale(1.2, 1.2)
plt.title(f'Evaluation Metrics for Training and Test Sets ({TARGET_COLUMN})')
plt.show()
...
plt.xlabel(f'Actual {TARGET_COLUMN} Content (%)')
plt.ylabel(f'Predicted {TARGET_COLUMN} Content (%)')
plt.title(f'PLS Regression: Actual vs Predicted {TARGET_COLUMN}')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
output_filename = f'PLS_{TARGET_COLUMN}_Autoscaling_NoCARS_NoSG_{timestamp}.xlsx'
