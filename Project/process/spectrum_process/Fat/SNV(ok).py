...
def snv(X):
    """
    對每個樣本應用 Standard Normal Variate (SNV) 轉換
    X: 輸入數據 (樣本數 x 波長數)
    """
    ...
    for i in range(X.shape[0]):
        ...
        X_snv[i, :] = (sample - mean) / std if std > 0 else sample
    return X_snv
...
print(" Standard Normal Variate (SNV) 預處理完成")

# === 步驟 6: 使用 SNV 處理後的數據 ===
X_train_selected = X_train_snv
X_test_selected = X_test_snv
print(f" 使用 SNV 處理後數據，訓練集形狀: {X_train_selected.shape}, 測試集形狀: {X_test_selected.shape}")
...
output_filename = f'PLS_Fat_NoCARS_SNV_{timestamp}.xlsx'
