...
def msc(X, reference=None):
    """
    多重散射校正（MSC）處理，對每個樣本的光譜進行校正。
    參數：
        X: DataFrame 或 NumPy 陣列，行是樣本，列是波長點
        reference: 參考光譜（一維陣列），默認為輸入數據的均值光譜
    返回：
        X_msc: 校正後的 DataFrame
    """
    ...
    for i in range(X.shape[0]):
        ...
        coeffs, _, _, _ = np.linalg.lstsq(A, X[i], rcond=None)
        a_i, b_i = coeffs
        X_msc[i] = (X[i] - b_i) / a_i if a_i != 0 else X[i]
    
    return pd.DataFrame(X_msc, index=indices, columns=columns)
...
# === 步驟 5: MSC 預處理 ===
train_mean_spectrum = np.mean(X_train, axis=0)
X_train_msc = msc(X_train, reference=train_mean_spectrum)
X_train = pd.DataFrame(X_train_msc, index=X_train.index, columns=X_train.columns)
X_test_msc = msc(X_test, reference=train_mean_spectrum)  # 使用訓練集均值光譜
X_test = pd.DataFrame(X_test_msc, index=X_test.index, columns=X_test.columns)
print(" NIR 光譜 MSC 預處理完成")

# === 步驟 6: 使用 MSC 數據 ===
X_train_selected = X_train
X_test_selected = X_test
print(f" 使用MSC 數據，訓練集形狀: {X_train_selected.shape}, 測試集形狀: {X_test_selected.shape}")
...
base_filename = 'PLS_Fat_Standardize_MSC_NoCARS_NoSG'
counter = 1
