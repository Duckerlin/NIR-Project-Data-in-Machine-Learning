...
# 定義統一參數
PARAMS = {
    'WAVELENGTH_RANGE': (850, 1200),  # 波長範圍 (min, max)
...
mask_mcs = mask_mcs[:len(mask_md)]  # 確保長度一致
...
print(" 跳過獨立標準化，將在步驟 5 結合 Savitzky-Golay 濾波與中心化")
...
# === 步驟 5: 光譜預處理（結合 Savitzky-Golay 濾波與中心化） ===
...
print(" 光譜預處理完成：Savitzky-Golay 濾波與中心化結合")

# === 步驟 6: 移除 CARS，保留完整波長數據 ===
# 直接使用預處理後的數據
X_train_selected = X_train_preprocessed
X_test_selected = X_test_preprocessed
print(f" 使用完整波長數據，訓練集形狀: {X_train_selected.shape}, 測試集形狀: {X_test_selected.shape}")
...
output_filename = f'PLS_Fat_NoCARS_{timestamp}.xlsx'
