...
PARAMS = {
    ...
    'TRAIN_TEST_RATIO': 0.2,  # 測試集比例 (0.35 表示 65:35)
...
if not np.all(fat_mask):
    print(f" 檢測到脂肪含量異常值（應在 0-100%）：{Y[~fat_mask]}")
    X = X[fat_mask]
    Y = Y[fat_mask]
...
# === 步驟 3: NIR 光譜預處理（移除標準化，移到步驟 5 結合中心化） ===
...
table.scale(1.2, 1.2)
plt.title('Evaluation Metrics for Training and Test Sets (Fat)')
plt.tight_layout()
plt.show()
...
plt.xlabel('Actual Fat Content (%)')
plt.ylabel('Predicted Fat Content (%)')
plt.title('PLS Regression: Actual vs Predicted Fat')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
base_filename = 'PLS_Fat_Centering_MAS_NoCARS'
counter = 1
