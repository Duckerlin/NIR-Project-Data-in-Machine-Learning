...
# 定義統一參數
PARAMS = {
    'WAVELENGTH_RANGE': (850, 1100),  # 波長範圍 (min, max)
    ...
    'TRAIN_TEST_RATIO_STR': '65:35',  # 訓練-測試比例字串表示
}
...
X_train = pd.DataFrame(X_train_norm, columns=X.columns, index=X_train.index)
X_test_norm = normalize(X_test)
X_test = pd.DataFrame(X_test_norm, columns=X.columns, index=X_test.index)
print(" NIR 光譜 Normalization 完成")

# === 步驟 5: 使用Normalization預處理之數據 ===
X_train_selected = X_train
X_test_selected = X_test
print(f" 使用Normalization 數據，訓練集形狀: {X_train_selected.shape}, 測試集形狀: {X_test_selected.shape}")
...
base_filename = 'PLS_Fat_SPXY_Standardize_Normalize_NoCARS_NoSG'
counter = 1
