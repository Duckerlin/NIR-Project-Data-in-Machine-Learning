...
# 定義統一參數
PARAMS = {
    'WAVELENGTH_RANGE': (650, 1200),  # 波長範圍 (min, max)
...
table.scale(1.2, 1.2)
plt.title('Evaluation Metrics for Training and Test Sets (Ash)', fontsize=16, pad=10)
plt.tight_layout()
plt.show()
...
plt.xlabel('Actual Ash Content (%)', fontsize=14)
plt.ylabel('Predicted Ash Content (%)', fontsize=14)
plt.title('PLS Regression: Actual vs Predicted Ash', fontsize=16, pad=10)
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
base_filename = 'PLS_Ash_Standardize_SGS_MAS_NoCARS'
counter = 1
