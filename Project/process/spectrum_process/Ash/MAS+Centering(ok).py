...
if not np.all(ash_mask):
    print(f" 檢測到Ash含量異常值（應在 0-50%）：{Y[~ash_mask]}")
    X = X[ash_mask]
    Y = Y[ash_mask]
    print(f" 移除Ash含量異常值後，剩餘樣本數：{X.shape[0]}")
    print(f" 移除後目標值 Y 描述:", pd.Series(Y).describe())

# 繪製Ash含量分佈直方圖
plt.figure(figsize=(8, 6))
plt.hist(Y, bins=20, edgecolor='black', alpha=0.7)
plt.xlabel('Ash Content (%)', fontsize=14)
plt.ylabel('Frequency', fontsize=14)
plt.title('Distribution of Ash Content', fontsize=16, pad=10)
...
plt.title('PLS Regression: Actual vs Predicted Ash')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
base_filename = 'PLS_Ash_Centering_MAS_NoCARS'
counter = 1
