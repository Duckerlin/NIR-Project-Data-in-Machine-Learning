...
# UVE 波長選擇函數
def uve_wavelength_selection(X, y, n_components, n_noise_vars=50, cv_folds=5):
...
PARAMS = {
    ...
    'DT': {
        ...
        'TRAIN_TEST_RATIO': 0.2,
        'TRAIN_TEST_RATIO_STR': '80:20',
        'MAX_DEPTH_RANGE': [3,7,10,15,20],
        'MIN_SAMPLES_SPLIT_RANGE': [2,3,7,9,10,20],
        'MAS_WINDOW_SIZE': 13
    },
    'SVM': {
        'WAVELENGTH_RANGE': (850, 1099.5),
...
for model_name in ['PLS', 'DT', 'SVM', 'RF']:
    ...
    print(f"  {model_name} 光譜預處理完成：移動平均平滑與中心化結合")

    # === 步驟 6: 特徵選擇與模型訓練 ===
    if model_name == 'PLS':
        # 無信息變量消除（UVE）波長選擇
        selected_wavelength_indices = uve_wavelength_selection(X_train_preprocessed.values, Y_train, 
