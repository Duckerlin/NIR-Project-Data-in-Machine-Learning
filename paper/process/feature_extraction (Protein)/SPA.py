...
PARAMS = {
    ...
    'SPA_MAX_WAVELENGTHS': 24,  # SPA 最大選擇波長數
}
...
try:
    ...
    print("  無法找到 nir_data.csv，請確認檔案存在於指定路徑。")
    exit()
...
try:
    ...
    print("  無法找到 trad_data.xlsx，請確認檔案存在於指定路徑。")
    exit()
...
if len(nir_final_df) != 1000:
    print(f"  波長點數 {len(nir_final_df)} 不等於預期 1000，檢查數據格式")
...
if not np.all(protein_mask):
    print(f"  檢測到蛋白質含量異常值（應在 0-50%）：{Y[~protein_mask]}")
    X = X[protein_mask]
    Y = Y[protein_mask]
...
plt.title('Distribution of Protein Content', fontsize=16, pad=10)
...
def spxy(X, Y, test_size=PARAMS['TRAIN_TEST_RATIO']):
    if not isinstance(X, pd.DataFrame):
        print("  X is not a DataFrame, converting to DataFrame with default indices")
        X = pd.DataFrame(X, index=[f'sample_{i}' for i in range(X.shape[0])], 
                         columns=[f'wavelength_{i}' for i in range(X.shape[1])])
    ...
    X_train = pd.DataFrame(X_array[selected], index=X.index[selected], columns=X.columns)
    Y_train = Y[selected].ravel()
    X_test = pd.DataFrame(X_array[remaining], index=X.index[remaining], columns=X.columns)
...
X_train_centered = pd.DataFrame(X_train_centered, index=X_train.index, columns=X_train.columns)
X_test_centered = center(X_test)
X_test_centered = pd.DataFrame(X_test_centered, index=X_test.index, columns=X_test.columns)
print("  NIR 光譜中心化完成")

# === 步驟 6: SPA 特徵波長選擇 ===
selected_wavelength_indices = spa(X_train_centered.values, Y_train, max_wavelengths=PARAMS['SPA_MAX_WAVELENGTHS'])
wavelengths = nir_final_df['Wavelength'].values
selected_wavelengths = wavelengths[selected_wavelength_indices]
X_train_selected = X_train_centered.values[:, selected_wavelength_indices]
X_test_selected = X_test_centered.values[:, selected_wavelength_indices]
...
# 繪製選擇的波長（修復 scaler_x.inverse_transform 的輸入形狀）
plt.figure(figsize=(10, 6))
plt.plot(wavelengths, scaler_x.inverse_transform(X_train.mean().values.reshape(1, -1))[0], label='Mean Spectrum')
plt.scatter(selected_wavelengths, scaler_x.inverse_transform(X_train.mean().values.reshape(1, -1))[0][selected_wavelength_indices], 
           color='red', marker='o', label='SPA Selected Wavelengths')
...
table.scale(1.2, 1.2)
plt.title('Evaluation Metrics for Training and Test Sets (Protein)', fontsize=16, pad=10)
plt.tight_layout()
plt.show()
...
plt.xlabel('Actual Protein Content (%)', fontsize=14)
plt.ylabel('Predicted Protein Content (%)', fontsize=14)
plt.title('PLS Regression: Actual vs Predicted Protein (SPA)', fontsize=16, pad=10)
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
base_filename = 'PLS_Protein_Standardize_Centering_SPA'
counter = 1
